...
import (
	...
	"github.com/treasurenet/x/erc20/types"
)
...
func NewConvertCoinCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) == 2 {
				receiver = args[1]
				if err := treasurenet.ValidateAddress(receiver); err != nil {
...
func NewConvertERC20Cmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			contract := args[0]
			if err := treasurenet.ValidateAddress(contract); err != nil {
...
{
  "description": "staking, gas and governance token of the Treasurenet testnets"
  "denom_units": [
		{
			"denom": "atreasurenet",
			"exponent": 0,
			"aliases": ["atto treasurenet"]
		},
		{
			"denom": "treasurenet",
			"exponent": 18
	...
	cmd.Flags().String(cli.FlagDeposit, "1atreasurenet", "deposit of proposal")
