...
import (
	...
	"github.com/treasurenet/testutil"
...
func (suite *KeeperTestSuite) TestHookBeforeAirdropStart() {
	...
	airdropStartTime := time.Now().UTC().Add(time.Hour)
	params := suite.app.ClaimsKeeper.GetParams(suite.ctx)
	...
	coins = suite.app.ClaimsKeeper.GetClaimableAmountForAction(suite.ctx, claimsRecord, types.ActionVote, suite.app.ClaimsKeeper.GetParams(suite.ctx))
	suite.Require().Equal(coins, sdk.NewInt(250)) // 1/4th of the claimable

	_, err := suite.app.ClaimsKeeper.ClaimCoinsForAction(suite.ctx, addr1, claimsRecord, types.ActionVote, params)
	suite.Require().NoError(err)
...
func (suite *KeeperTestSuite) TestClawbackEmptyAccountsAirdrop() {
	...
	tests := []struct {
		...
		{
			name:           "address active",
			address:        "eth1qql8ag4cluz6r4dz28p3w00dnc9w8ueuafmxps",
			sequence:       1,
		...
		},
		{
			...
			address:        "eth1x2w87cvt5mqjncav4lxy8yfreynn273xn5335v",
			sequence:       0,
			expectClawback: true,
			claimsRecord: types.ClaimsRecord{
				InitialClaimableAmount: sdk.NewInt(100),
	...
	t := []struct {
	...
	}{
		{
			fn: func() {
				...
				suite.Require().Equal(sdk.Coins{sdk.NewInt64Coin("atreasurenet", 2)}, balances)
			},
		},
		{
			fn: func() {
				...
				suite.Require().Equal(sdk.Coins{sdk.NewInt64Coin("atreasurenet", 2+1)}, balances)
			},
		},
		{
			fn: func() {
				...
				suite.Require().Equal(sdk.Coins{sdk.NewInt64Coin("atreasurenet", 2+3)}, balances)
			},
		},
		{
			fn: func() {
