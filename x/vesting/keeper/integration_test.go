...
import (
	...
	"github.com/treasurenet/x/vesting/types"
)
...
// Example:
// 21/10 Employee joins Treasurenet and vesting starts
...
func delegate(clawbackAccount *types.ClawbackVestingAccount, amount int64) error {
	...
	s.Require().NoError(err)
	//
	val, err := sdk.ValAddressFromBech32("ethvaloper1z3t55m0l9h0eupuz3dp5t5cypyv674jjn4d6nn")
	s.Require().NoError(err)
