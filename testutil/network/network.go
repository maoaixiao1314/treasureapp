...
// NewAppConstructor returns a new Treasurenet AppConstructor
func NewAppConstructor(encodingCfg params.EncodingConfig) AppConstructor {
	return func(val Validator) servertypes.Application {
		return app.NewTreasurenet(
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	if !treasurenet.IsValidChainID(cfg.ChainID) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "treasurenetcli")
		...
		customAppTemplate, _ := config.AppConfig(treasurenet.AttoPhoton)
