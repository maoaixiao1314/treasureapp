...
- **Tendermint Key**: This is a unique key used to sign consensus votes.
  - It is associated with a public key `ethvalconspub` (Get this value with `treasurenetd tendermint show-validator`)
  - It is generated when the node is created with `treasurenetd init`.
- **Application key**: This key is created from `treasurenetd` and used to sign transactions. Application keys are associated with a public key prefixed by `ethpub` and an address prefixed by `eth`. Both are derived from account keys generated by `treasurenetd keys add`.
