...
var (
	...
	LastValidatorTatPowerKey  = []byte{0x13} // prefix for each key to a validator index, for bonded validators TAT
	LastTatTotalPowerKey      = []byte{0x14} // prefix for the tat total power
...
func GetTatTokensKey(operatorAddr sdk.ValAddress) []byte {
	return append(TatTokensKey, address.MustLengthPrefix(operatorAddr)...)
}
func GetNewTokensKey(operatorAddr sdk.ValAddress) []byte {
	return append(NewTokensKey, address.MustLengthPrefix(operatorAddr)...)
}
...
// GetLastValidatorPowerKey creates the tat bonded validator index key for an operator address
func GetLastValidatorTatPowerKey(operator sdk.ValAddress) []byte {
	return append(LastValidatorTatPowerKey, address.MustLengthPrefix(operator)...)
}

// GetLastValidatorPowerKey creates the tat bonded validator index key for an operator address
func GetLastValidatorUnitPowerKey(operator sdk.ValAddress) []byte {
	return append(LastValidatorUnitPowerKey, address.MustLengthPrefix(operator)...)
}
