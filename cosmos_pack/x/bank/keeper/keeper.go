...
// SendCoinsFromModuleToModule transfers coins from a ModuleAccount to another. SendCoinsFromModuleToModule 将硬币从一个 ModuleAccount 转移到另一个。
// It will panic if either module account does not exist. // 如果任一模块帐户不存在，它就会发生恐慌。
